...
try:
    ...
    if not premium_session:
        LOGGER.error("Can't initialize User Session. Please regenerate USER_SESSION_STRING")
    else:
        premium_session.start()
        if (premium_session.get_me()).is_premium:
            if not LEECH_LOG:
                LOGGER.error("You must set LEECH_LOG for uploads. README.md is there for a reason. Exiting now.")
                try: premium_session.send_message(OWNER_ID, "You must set LEECH_LOG for uploads, README.md is there for a reason, Read it properly. Exiting Now...")
